...
void rotarAux(nat clave, char tipo[2], TAbb &abb){
    if (natInfo(raiz(abb)) == clave){
        ...
        rotarAux(clave,tipo,abb->izq);
    }else if(clave > natInfo(raiz(abb))){
        rotarAux(clave,tipo,abb->der);
    }
}

TAbb rotar(nat clave, char tipo[2], TAbb abb){
    if (abb != NULL && !esh(abb)){
        rotarAux(clave,tipo,abb);    
    }
    return abb;
}
...
bool esHoja(TAbb abb){
    if (abb != NULL && abb->der == NULL &&  abb->izq == NULL){
        return true;
    }else{
        return false;
    }
